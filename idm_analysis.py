...
# _nWithinAcceptance  		  = 0
# _nWithinTPC		  		  = 0
# _twoTracks         		  = 0
# _twoTruthTracks   		  = 0
# _allTracks		  		  = 0
# _nMatchingTracks 		  = 0
# _nMatchingTracksReffited = 0
# _nMatchingVertices		  = 0
# _matchingVerticesTPC		  = 0
# _twoRecoLeptons    		  = 0
# _onlyMarlinTrkTracks		  = 0
# _onlyRefittedTracks		  = 0
# _nPFOs					  = 0
# _nCorrPID				  = 0
# _nEvSecVertices		  	  = 0
# _nSecVertices		  	  = 0
...
def run_analysis(reader):
	counter = counters.Counters()
	for event in reader:
		process_event(event,counter)
	reader.close()
	print_output(counter)
	histo_array = print_histos()
	save_to_file(histo_array)

def process_event(event,counter):
			...
			for trackRel in trackToMCLinkCollection:
				...
				and trackRel.getTo().getCharge() * utils.getTrackCharge(trackState) > 0:
					...
					if firstSimHit and secondSimHit:
						...
						d12 = 0
						print iEvt, ' event: one of the SimTrackerHits not found!'
				...
				#and vtx_dist < 100.:
					...
					# _nMatchingTracks+=1
					counter.increment("_nMatchingTracks", True, 1)
					matchingTracksInEvent+=1
					# print iEvt, particle.getPDG(), matchingTracksInEvent
					break
	'''
	for track in trackCollection:
		...
		if firstSimHit and secondSimHit:
			...
			d12 = 0
			print iEvt, ' event: at least one of the SimTrackerHits in track not found!'
			print 'firstSimHit:', firstSimHit
			print 'secondSimHit:',secondSimHit
			print ''
	...
	for particle in mcCollection:
		...
		if( particle.getGeneratorStatus() == gen_status):
			...
			for trackRel in refitTrackToMCLinkCollection:
				...
				if hits.size() < 4:
					print('')
					print(iEvt, "REFITTED TRACK CONTAINS < 4 HITS")
					print('')
				...
				#and vtx_dist < 100.:
					...
					counter.increment("_nMatchingTracksReffited", True, 1)
	...
	if 'LLPVertices' in event.getCollectionNames():
		...
		for vtx in vertexCollection:
			...
			pos = vtx.getPosition()
			vtxVsR_reco.Fill( utils.R(pos[0],pos[1]) )
			vtxVsL_reco.Fill( utils.R3(pos[0],pos[1],pos[2]) )
			...
			if dist < dist0:
				...
				bestVtx = vtx
			if dist > 30:
				fakeVertices.Fill( pos[2], utils.R(pos[0],pos[1]) )
		...
		if dist0 < 30:
			recoVertices.Fill( pos0[2], utils.R(pos0[0],pos0[1]) )
			...
			# if True:
				...
				if True:
					...
					if utils.R(vtx_cand.getPosition()[0],vtx_cand.getPosition()[1]) > 350:
						corrCentresRefDistsVsR.Fill(centresRefsDists, utils.R(vtx_cand.getPosition()[0],vtx_cand.getPosition()[1]))
...
def print_histos():
	...
	trackEffVsR = ROOT.TH1F('trackEffVsR', 'Track reconstruction efficiency', _nbins, 0., _TPCRmax)
	...
	vtxVsR_reco.SetXTitle('utils.R [mm]')
	...
	'''
	trackEffRVsZ = ROOT.TH2F('trackEffRVsZ', 'Track reconstruction efficiency', _nbins, -_TPCZmax, _TPCZmax, _nbins, 0., _TPCRmax)
	...
	vtxEffRVsZ_true.SetYTitle('utils.R [mm]')
	...
	trackEffRVsZ_true.SetYTitle('utils.R [mm]')
	...
	recoVertices.SetYTitle('utils.R [mm]')
	...
	fakeVertices.SetYTitle('utils.R [mm]')
	...
	trackEffiPvsR_true.SetYTitle('utils.R [mm]')
...
if __name__ == '__main__':
	...
	# track efficiency
	trackEffVsR_true = ROOT.TH1F('trackEffVsR_true', 'True vertices', _nbins, 0., _TPCRmax)
	trackEffVsR_reco = ROOT.TH1F('trackEffVsR_reco', 'N matching reco. tracks', _nbins, 0., _TPCRmax)
	...
	trackEffRVsZ_true = ROOT.TH2F('trackEffRVsZ_true', 'True vertices', _nbins, -_TPCZmax, _TPCZmax, _nbins, 0., _TPCRmax)
	trackEffRVsZ_reco = ROOT.TH2F('trackEffRVsZ_reco', 'N matching reco. tracks', _nbins, -_TPCZmax, _TPCZmax, _nbins, 0., _TPCRmax)
	...
	trackEffiPvsR_true = ROOT.TH2F('trackEffiPvsR_true', '', 15, 0., 15., _nbins, 0., _TPCRmax)
	...
	# 2-D vertices plots
	recoVertices = ROOT.TH2F('recoVertices', 'Number of found vertices', _nbins, -_TPCZmax, _TPCZmax, _nbins, 0., _TPCRmax)
	fakeVertices = ROOT.TH2F('fakeVertices', 'Vertices not matched to MC', _nbins, -_TPCZmax, _TPCZmax, _nbins, 0., _TPCRmax)
	vtxEffRVsZ_true = ROOT.TH2F('vtxEffRVsZ_true', 'True vertices', _nbins, -_TPCZmax, _TPCZmax, _nbins, 0., _TPCRmax)
	vtxEffRVsZ_reco = ROOT.TH2F('vtxEffRVsZ_reco', 'True vtx corresponding to reco. vtx', _nbins, -_TPCZmax, _TPCZmax, _nbins, 0., _TPCRmax)
	...
	vtxTrksD01VsD02 = ROOT.TH2F('vtxTrksD01VsD02', 'd_{0} (w.utils.R.t. IP) of tracks in vtx', 50, 0, 2000, 50, 0, 2000)
	vtxTrksZ01VsZ02 = ROOT.TH2F('vtxTrksZ01VsZ02', 'z_{0} (w.utils.R.t. IP) of tracks in vtx', 50, 0, 2500, 50, 0, 2500)
	...
	vtxRefPhiVsLhPhi = ROOT.TH2F('vtxRefPhiVsLhPhi', 'Ref. point and last hit phi w.utils.R.t. vtx phi', 50, -0.5, 0.5, 50, -0.5, 0.5)
	...
	vtxTrksD0AtIP = ROOT.TH1F('vtxTrksD0AtIP', 'd_{0} (w.utils.R.t. IP) of tracks in vtx', 50, 0, 25)
	vtxTrksZ0AtIP = ROOT.TH1F('vtxTrksZ0AtIP', 'z_{0} (w.utils.R.t. IP) of tracks in vtx', 50, 0, 25)
