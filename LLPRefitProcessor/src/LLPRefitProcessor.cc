...
LLPRefitProcessor aLLPRefitProcessor ;


LLPRefitProcessor::LLPRefitProcessor() : Processor("LLPRefitProcessor") {

  // modify processor description
  _description = "LLPRefitProcessor refits an input track collection, producing a new collection of tracks." ;
...
void LLPRefitProcessor::init() {
  ...
  streamlog_out(DEBUG) << "   Initialising detector geometry and bField...  "  << std::endl ;
  ...
  streamlog_out(DEBUG) << "   Detector initialised, bField set  "  << std::endl ;
...
void LLPRefitProcessor::processRunHeader( LCRunHeader* ) {

  ++_n_run ;
}

void LLPRefitProcessor::processEvent( LCEvent * evt ) {
  ...
  if( input_track_col != 0 ){
	...
	if( _initialTrackState < 0 ) { // initialize the track from three hits
	  ...
	  error = MarlinTrk::createFinalisedLCIOTrack(marlinTrk, trkHits, refittedTrack, fit_direction, &pre_fit , _bField, _maxChi2PerHit);
        ...
        streamlog_out(ERROR) << "LLPRefitProcessor::processEvent: EVENT: << " << evt->getEventNumber() << " >> exception caught and rethown. Track = "
      ...
      if( hits_in_fit.size() < 3 ) {
        streamlog_out(DEBUG6) << "LLPRefitProcessor: Less than 3 hits in fit: number of hits =  " << trkHits.size() << std::endl;
      }
...
void LLPRefitProcessor::check( LCEvent * ) {
  // nothing to check here - could be used to fill checkplots in reconstruction processor
}


void LLPRefitProcessor::end(){

  streamlog_out(DEBUG) << "LLPRefitProcessor::end()  " << name()
  << " processed " << _n_evt << " events in " << _n_run << " runs "
  << std::endl ;

}

LCCollection* LLPRefitProcessor::GetCollection( LCEvent * evt, std::string colName ){
...
LCRelationNavigator* LLPRefitProcessor::GetRelations(LCEvent * evt , std::string RelName ) {

  LCRelationNavigator* nav = 0 ;

  try{
    nav = new LCRelationNavigator(evt->getCollection( RelName.c_str() ));
    streamlog_out( DEBUG2 ) << "LLPRefitProcessor --> " << RelName << " track relation collection in event = " << nav << std::endl;
  }
  catch(DataNotAvailableException &e){
    streamlog_out( DEBUG2 ) << "LLPRefitProcessor --> " << RelName.c_str() << " track relation collection absent in event" << std::endl;
  }
